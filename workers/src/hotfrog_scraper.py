...
async def scrape_hotfrog(
    ...
    """
    Scrape Hotfrog for business listings.
    """
    extracted_data = []
    
    query = f"site:hotfrog.in {keyword} {city}"
    url = "https://html.duckduckgo.com/html/"
    
    try:
        logger.info(f"Hotfrog: Searching for '{query}'")
        import urllib.parse
        ...
        logger.info(f"Hotfrog: Found {len(extracted_data)} listings")
        return extracted_data
        
    except Exception as e:
        logger.error(f"Hotfrog scraping error: {e}")
        return extracted_data
